...
const Add = ({token}) => {
  ...
  const [category,setCategory] = useState ("Icecake");
  const [subCategory, setSubCategory ] = useState ("Plain");
      ...
      <div className='flex flex-col sm:flex-row gap-2 w-full sm:gap-8'>
        <div >
          ...
          <select onChange={(e)=>setCategory(e.target.value)} className='w-full px-3 py-2'>
            <option value="Icecake">Icecake</option>
            <option value="Buttercreamcake">Buttercreamcake</option>
            <option value="Ganache">Ganaches</option>
          </select>
        </div>
        ...
        <div >
          ...
          <select onChange={(e)=>setSubCategory(e.target.value)} className='w-full px-3 py-2'>
            <option value="Plaincake">Plaincake</option>
            <option value="Themecake">Themecake</option>
            <option value="Fondantcake">Fondantcake</option>
          </select>
        </div>

        <div>
          <p className='mb-2 '>Product price</p>
          <input onChange={(e)=>setPrice(e.target.value)} className='w-full px-3 py-2 sm:w-[120px]' type="Number" placeholder='2' />
        </div>
      </div>
        ...
        <div >
          ...
          <div className='flex gap-3'>
            ...
            <div onClick={()=>setSizes(prev => prev.includes("1.5Kg") ? prev.filter(item => item !== "1.5Kg") : [...prev,"1.5Kg"] )}>
              <p className={`${sizes.includes("1.5Kg") ? "bg-pink-100" : "bg-slate-200" } px-3 py-1 cursor-pointer`} >1.5Kg</p>
            </div>
